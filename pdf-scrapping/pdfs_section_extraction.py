...
USEFUL_SECTIONS = ["executive summary", "summary", "key takeaway", "conclusion",
                   "abstract"]

SECTIONS = ["acknowledgment", "acknowledgement",
            "reference", "appendix", "appendic", "declaration", "note", "literature", "endnote",
            "correspondence",
...
def contains_useful_section_prefix(text):
    return contains_keywords(text, USEFUL_SECTIONS, startswith=False)

def contains_other_section_keywords(text):
    return contains_keywords(text, SECTIONS, startswith=True)

def extract_summary_part(filename):
    pages = extract_text_from_pdf(filename)
    text_to_use = []
    for page in range(len(pages)-1, -1, -1):
        if contains_useful_section_prefix(pages[page]):
            all_text = "\n\n".join(pages[page:])
            start = False
            for line in all_text.split("\n"):
                line_to_check = line.replace(" ", "").strip() if line.isupper() else line
                if contains_useful_section_prefix(line) or contains_useful_section_prefix(line_to_check):
                    start = True
